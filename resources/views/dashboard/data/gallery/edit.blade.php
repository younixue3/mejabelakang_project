...
<main>
    <form class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8" action="{{route('gallery.update', $gallery->id)}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="bg-white shadow-xl p-10 grid gap-y-3">
            <label>
                ...
                <input class="px-2.5 py-1 w-1/2 border-2 text-lg focus:outline-none rounded-xl" type="text" id="title" name="title" value="{{$gallery->title}}">
            </label>
            <label>
                <div class="font-semibold">
                    Isi Konten
